...
class AIChatAgent<Env = unknown, State = unknown> extends Agent<Env, State> {
	...
	// options includes abortSignal and clientTools for dynamic tool registration
	async onChatMessage(
...
// Options passed to onChatMessage handler
...
Converts client tool schemas to AI SDK tool format for use in your Agent's tools configuration.

```ts
import { createToolsFromClientSchemas } from "agents/ai-chat-agent";
...
These tools have no `execute` function. When the AI model calls them, the tool call is sent back to the client for execution.
...
type UseAgentChatOptions = Omit<
	Parameters<typeof useChat>[0] & {
		...
		// Tools requiring confirmation (manual override)
		toolsRequiringConfirmation?: string[];
		// Enable automatic tool resolution
		experimental_automaticToolResolution?: boolean;
		// Auto-send after all confirmations are resolved
		autoSendAfterAllConfirmationsResolved?: boolean;
		// Enable stream resumption (default: true)
		resume?: boolean;
		// Customize request before sending messages
...
// Tool definition with client-side execution
...
// Options for prepareSendMessagesRequest callback
