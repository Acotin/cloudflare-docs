...
The Task Management API provides lifecycle tracking for long-running operations in your Agent. Tasks automatically handle progress updates, event streaming, cancellation, and persistence - making them ideal for operations like API calls, file processing, or AI agent runs that need real-time status updates.

#### Creating tasks

Tasks are created using the `@task()` decorator or the `this.task()` method. The decorator provides a cleaner API, while the method offers more flexibility.
...
export class YourAgent extends Agent<Env> {
  // Option 1: Using the @task() decorator (recommended)
  @task({ timeout: "5m" })
  ...
  // Option 2: Using this.task() method directly
  async startCustomTask(input: unknown) {
    ...
    return { taskId: taskHandle.id, status: taskHandle.status };
  }

  // The method that will be executed as a task
  async customMethod(input: unknown, ctx: TaskContext) {
...
Each state change persists to SQLite and broadcasts to connected clients in real-time.

#### TaskContext API

The `TaskContext` parameter in task methods provides methods for progress tracking and cancellation:
...
export class YourAgent extends Agent {
  @task({ timeout: "5m" })
  async processFile(input: { fileUrl: string }, ctx: TaskContext) {
    // Emit custom events
    ctx.emit("download_started", { url: input.fileUrl });
    ctx.setProgress(10);

    const file = await this.downloadFile(input.fileUrl);
    ...
    ctx.emit("processing", { size: file.size });
    ctx.setProgress(50);

    const result = await this.processData(file);
...
Access and manage tasks using the `this.tasks` property:
...
class Agent<Env, State = unknown> {
  tasks: {
    ...
    list(options?: { status?: TaskStatus }): Task[];
...
export class YourAgent extends Agent {
  ...
  async listActiveTasks() {
    ...
    const pending = this.tasks.list({ status: "pending" });
  ...
  async cleanupOldTasks() {
    ...
    const removed = await this.tasks.cleanup(24 * 60 * 60 * 1000);
    return { removed };
  }
}
```
...
interface TaskOptions {
  // Timeout duration as string or milliseconds
  timeout?: string | number; // Examples: "30s", "5m", "2h", or 300000
...
The `Task` interface represents a tracked task:
...
interface Task<TResult = unknown> {
  ...
  // Task result (available when status is "completed")
  result?: TResult;
...
Track task progress in real-time from React components using the `useTask` hook:
...
function TaskProgress({ taskId }: { taskId: string }) {
  ...
  return (
    <div>
      ...
      {task.status === "failed" && <div>Error: {task.error}</div>}
    </div>
  );
}
```
...
Tasks can also trigger [Cloudflare Workflows](/workflows/) for durable, long-running operations. This pattern combines the client API simplicity of tasks with the durability and automatic retry capabilities of workflows:
...
export class YourAgent extends Agent<Env> {
  // Use @callable() for workflows (not @task())
  @callable()
...
```tsx
// Works for both @task() and workflow() methods
const task = await agent.call("deepAnalysis", [{ repoUrl: "..." }]);
console.log(task.id, task.status); // Real-time updates work the same way
```

Refer to [Run Workflows](/agents/api-reference/run-workflows/) for more details on integrating Agents with Workflows.
